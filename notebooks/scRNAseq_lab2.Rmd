...
# pdf("explain_pcs.pdf")
scater::plotExplanatoryPCs(explan_pcs)
# dev.off()
```

From the plot above, we can see that, four shown meta.data (nFeature_RNA, nCount_RNA, S.Score, Phase) explain more than 10% of the variation in the top PC, we may regress them out duing `SCTransform`

  - nCount_RNA (i.e. total cell UMI count) are in the NB GLM model by default, so no need to add again, nFeature_RNA is highly positively correlated with nCount_RNA, we shouldn't add it either
    - The reason that nCount_RNA and nFeature_RNA still explain a big portion of the data variation is because SCTransform only removes the technical component of the total cell UMI count/seq depth, 
    - the biological component of sequencing depth still remains (e.g. seq depth may differ between different cell types...)
  - We already know from last tutorial Phase is not accurate, so we will regres out S.Score only. Though sometimes, one may want to keep the cell cycle info in the data.
  - If you want to, you can regress out percent.mit as well, it is often correlated with cell quality.
